...
const options = {
  swaggerDefinition: {
    info: {
      title: "Swagger API Documentation - Assignment 08",
      description: "Created by: Muthu Selvam",
      version: "3.0.0"
    },
  },
  apis: ["app.js"],
 ...
 *     description: Get customers from the database based on the provided country.
 *     parameters:
 *       - in: body
 *         name: body
  ...
  (req, res) => {
    pool
      ...
      .then((conn) => {
        ...
        conn
          .query("SELECT * FROM customer WHERE CUST_COUNTRY=?", [country]) // Pass country as a parameter
          .then((rows) => {
            res.json(rows);
            conn.end();
          })
 ...
 * /deleteCustomer:
 *   delete:
 *     summary: Delete customer by Customercode
 ...
 *     description: Update the customer in the database based on the provided customer code.
 *     parameters:
 *       - in: query
 *         name: customerCode
